...
We will test the performance of the common operations that we require to map terms to genes. For this purpose we will use two object types the native data.frame and the data.frame subclass data.table [https://cran.r-project.org/web/packages/data.table/index.html](https://cran.r-project.org/web/packages/data.table/index.html). We will also use the library fastmatch [https://cran.r-project.org/web/packages/fastmatch/index.html](https://cran.r-project.org/web/packages/fastmatch/index.html) and its native counterpart match.

No effort has been done on parallelization or implementation in C/C++.

```{r, warning = FALSE, message = FALSE}
## Loads raw annotations
raw_annotations <- TCGAome::load_kegg()@raw_annotations
...
3. Intersect using fastmatch

```{r, warning = FALSE, message = FALSE}
first_set = sample(1000, size = 100, replace = FALSE)
second_set = sample(1000, size = 100, replace = FALSE)
...
### XOR (assuming intersection and union is already computed)
...
kegg_term_clustering <- TCGAome::TermsClustering(
    kegg, kegg_enrichment, "cosine", significance_thr = 0.05, 
    adj_method = "none", max_clusters = 10)

representatives <- TCGAome::.select_representatives(
                  kegg_term_clustering@significant_results)
distance_matrix_repr <- as.dist(as.matrix(
                  kegg_term_clustering@distance_matrix)[
                      unlist(representatives$repr_term),
                      unlist(representatives$repr_term)])


res <- microbenchmark(
    "1" = TCGAome::get_significant_results(
                      kegg_enrichment, 0.05, "none"),
    "2" = TCGAome::get_term_distance_matrix(
                  kegg, "cosine",
                  terms_subset = kegg_term_clustering@significant_results$Term),
    "3" = TCGAome::.pam_clustering(
                  kegg_term_clustering@distance_matrix,
                  max_clusters = kegg_term_clustering@max_clusters),
    "4" = TCGAome::.multidimensional_scaling(
                  kegg_term_clustering@distance_matrix),
    "5" = vapply(
                  kegg_term_clustering@significant_results$Term,
                  FUN = function(term) {
                      TCGAome::get_term_freq(
                          kegg, term)
                  }, 
                  FUN.VALUE = numeric(1)),
    "6" = TCGAome::.select_representatives(
                  kegg_term_clustering@significant_results),
    "7" = distance_matrix_repr <- as.dist(as.matrix(
                  kegg_term_clustering@distance_matrix)[
                      unlist(representatives$repr_term),
                      unlist(representatives$repr_term)]),
    "8" = TCGAome::.multidimensional_scaling(
...
system.time(goa <- TCGAome::load_goa())
system.time(goa_enrichment <- TCGAome::get_enrichment(goa, gene_list = gene_list))
system.time(goa_term_clustering <- TCGAome::TermsClustering(goa, goa_enrichment, distance_measure = "cosine", significance_thr = 0.05, adj_method = "none", max_clusters = 10))
