...
descriptive_analysis <- function(X, Y, Z) {
    ...
    futile.logger::flog.info("Running descriptive analysis...")
    ...
    futile.logger::flog.info("Descriptive analysis finished.")
}
...
pca_analysis <- function(X, Y, Z) {
    ...
    futile.logger::flog.info("Running PCA analysis...")
    ...
    XY.pca <- prcomp(cbind2(X, Y), center = TRUE, scale. = TRUE)
    plot_simple_scatter(matrix = XY.pca, groups = Z$Tumor_type, output_dir = output_dir, file_name = "PCA.XY.png")

    futile.logger::flog.info("PCA analysis finished.")
}

### Hierarchical clustering
hclust_analysis <- function(X, Y, Z) {

    futile.logger::flog.info("Running hierarchical clustering analysis...")
    ...
    futile.logger::flog.info("Hierarchical clustering finished.")
}

### Regularized Canonical COrrelation Analysis (rCCA)
rcaa_analysis <- function(X, Y, Z) {
    futile.logger::flog.info("Running Regularized Canonical Correlation Analysis (rCCA)...")

    output_dir <- paste(RESULTS_FOLDER, "rCCA", sep = "/")
    dir.create(output_dir)

    # Calculates regularization factors with CV (M-folds and leave-one-out methods raise an error) Error in chol.default(Cxx) : the leading minor of order 407 is not positive definite grid1 <- seq(0,
    # 0.2, length = 51) grid2 <- seq(0.0001, 0.2, length = 51)
    cv_score <- mixOmics::tune.rcc(X, Y)  #, grid1 = grid1, grid2 = grid2, plt = FALSE) #, validation = 'loo')
    # Computes rCCA
    rcca_results <- mixOmics::rcc(X, Y, lambda1 = cv_score$opt.lambda1, lambda2 = cv_score$opt.lambda2)
    ...
    # network of relations between variables network(rcca_results, comp = 1:2, threshold = 0.2)

    # heatmap cim(rcca_results, comp = 1:2, xlab = 'proteins', ylab = 'genes', margins = c(5,6))

    futile.logger::flog.info("rCCA finished.")
...
rgcca_analysis <- function(X, Y, Z) {
    futile.logger::flog.info("Running Regularized Generalized Canonical Correlation Analysis (RGCCA)...")
    rgcca.results <- RGCCA::rgcca(list(X, Y), tau = "optimal", ncomp = 3)
    futile.logger::flog.info("RGCCA finished.")
...
spls_analysis <- function(X, Y, Z, topN = 5, selection_method = "loadings") {
    ...
    futile.logger::flog.info("Running sparse Partial Least Squares (sPLS) analysis...")

    if (!selection_method %in% c("correlation", "loadings")) {
        futile.logger::flog.error("Selection method '%s' non supported.", selection_method)
    ...
    # Runs sPLS
    spls_result <- mixOmics::spls(as.matrix(X), as.matrix(Y), ncomp = 3, mode = "regression")
    ...
    if (selection_method == "correlation") {
        ...
        # Selects top N variables with highest loadings, maximize variance in samples select topN t each component for positive vaues and for negative values separately
    ...
    futile.logger::flog.info("%d variables selected from RNAseq on sPLS results.", length(X_selected_variables))
    futile.logger::flog.info("%d variables selected from RPPA on sPLS results.", length(Y_selected_variables))
    futile.logger::flog.info("%d variables selected on sPLS results.", length(selected_variables))
    futile.logger::flog.info("sPLS finished.")
...
mcia_analysis <- function(X, Y, Z, topN = 5, cia.nf = 5) {
    futile.logger::flog.info("Running Multiple Co-Inertia Analysis (MCIA) ...")
    ...
    mcia_result <- omicade4::mcia(matrices_list, nsc = F, cia.nf = cia.nf)
    ...
    selected_variables_1 <- omicade4::topVar(mcia_result, axis = 1, topN = topN)
    ...
    selected_variables_2 <- omicade4::topVar(mcia_result, axis = 2, topN = topN)
    ...
    selected_variables_3 <- omicade4::topVar(mcia_result, axis = 3, topN = topN)
    ...
    futile.logger::flog.info("%d variables selected from RNAseq on MCIA results.", length(X_selected_variables))
    futile.logger::flog.info("%d variables selected from RPPA on MCIA results.", length(Y_selected_variables))
    futile.logger::flog.info("%d variables selected on MCIA results.", length(selected_variables))
    futile.logger::flog.info("MCIA finished.")
