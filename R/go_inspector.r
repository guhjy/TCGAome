...
read_go_ontology <- function()
{
  ...
  {
    #detach("package:ontoCAT", unload=TRUE)
    #detach("package:rJava", unload=TRUE)
    # Reads whole Gene ONtology tree
    options( java.parameters = "-Xms8G" )
    options( java.parameters = "-Xmx8G" )
    #.jinit()
    
    #.jcall(.jnew("java/lang/Runtime"), "J", "maxMemory")
    #go_tree <- getOntology("http://purl.obolibrary.org/obo/go/go-basic.obo")
    go_tree <<- getOntology("http://purl.obolibrary.org/obo/go.owl")
  }
  
  go_tree
}

# From the original terms list keeps only the most specific terms.
# All terms having any of its descendants included in the list is removed.
get_most_specific_terms <- function(terms_list)
{
  ...
  {
    children = sapply(getAllTermChildrenById(object=object, id = term_id), get_term_accession)
