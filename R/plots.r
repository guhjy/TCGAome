...
plot_scatter <-function(cluster_representatives, output_dir)
{
  ...
  flog.info("Created plot: %s", file_path)
}
...
plot_table <- function(cluster_representatives, output_dir){
  ...
  Cairo(file= file_path, type="png", units="in", width=10, height=10, pointsize=12, dpi=600)
  print ({
...
plot_heatmap <- function(matrix, tumor_types, output_dir, file_name){
  ...
  #col_tumor_type = as.character(tumor_types)
  #col_tumor_type[col_tumor_type == "BRCA"] <- red500
  #col_tumor_type[col_tumor_type == "OV"] <- green500

  Cairo(file=file_path, type="png", units="in", width=10, height=7, pointsize=12, dpi=600)
  print({
    heatmap.2(matrix, col=redgreen(75), scale="column", key=T, keysize=1.5,
              density.info="none", trace="none",cexCol=0.9, labRow=NA, RowSideColors=color_map)
  })
  dev.off()
...
mixomics_plot_individuals <- function (data, names, output_dir, file_name){
  ...
  #col_tumor_type = as.character(names)
  #col_tumor_type[col_tumor_type == "BRCA"] <- red500
  #col_tumor_type[col_tumor_type == "OV"] <- green500
...
mcia_plot <- function(mcia_result, phenotype, output_dir, file_name){
  ...
  Cairo(file=file_path, type="png", units="in", width=10, height=7, pointsize=12, dpi=600)
  print({
    plot(mcia_result, axes=1:2, phenovec=phenotype, sample.lab=FALSE, df.color=colors, sample.color=color_map,
