...
go_distance_matrix <- function (enrichment_results, measure = "Wang", gene_list = NULL, ont="BP", search_universe="human")
{
  ...
  for(term1 in colnames(go_distance_matrix)){
    for (term2 in rownames(go_distance_matrix))
    {
      #go_distance_matrix[term1, term2] = 1 - FUN(term1, term2, measure, gene_list = gene_list)
      go_distance_matrix[term1, term2] = FUN(term1, term2, measure, gene_list = gene_list, ont=ont, search_universe=search_universe)
    }
  }
  # converts similarity matrix into distance matrix
  go_distance_matrix = 1 - go_distance_matrix
  as.dist(go_distance_matrix)
}
