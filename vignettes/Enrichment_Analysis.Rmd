...
```{r, warning=FALSE, message=FALSE}
kegg_term_clustering <- TCGAome::TermsClustering(kegg_enrichment, "cosine", significance_thr = 0.05, adj_method = "none", max_clusters = 10)
...
kegg_term_clustering <- TCGAome::get_terms_clustering(kegg_enrichment, "cosine", significance_thr = 0.05, adj_method = "none")
```

Plot results:
```{r, warning=FALSE, message=FALSE, fig.width=7, fig.height=6}
TCGAome::plot_scatter(kegg_term_clustering, all = TRUE)
```

And do the same for other resources:
```{r, warning=FALSE, message=FALSE, fig.width=7, fig.height=6}
goa <- TCGAome::load_goa()
goa_enrichment <- TCGAome::get_enrichment(goa, gene_list = gene_list)
goa_term_clustering <- TCGAome::TermsClustering(goa_enrichment, "cosine", significance_thr = 0.05, adj_method = "none", max_clusters = 10)
TCGAome::plot_scatter(goa_term_clustering, all = TRUE)
```

```{r, warning=FALSE, message=FALSE, fig.width=7, fig.height=6}
hpo <- TCGAome::load_hpo()
hpo_enrichment <- TCGAome::get_enrichment(hpo, gene_list = gene_list)
hpo_term_clustering <- TCGAome::TermsClustering(hpo_enrichment, "cosine", significance_thr = 0.05, adj_method = "none", max_clusters = 10)
TCGAome::plot_scatter(hpo_term_clustering, all = TRUE)
```

```{r, warning=FALSE, message=FALSE, fig.width=7, fig.height=6}
omim <- TCGAome::load_omim()
omim_enrichment <- TCGAome::get_enrichment(omim, gene_list = gene_list)
omim_term_clustering <- TCGAome::TermsClustering(omim_enrichment, "cosine", significance_thr = 0.05, adj_method = "none", max_clusters = 10)
TCGAome::plot_scatter(omim_term_clustering, all = TRUE)
```
